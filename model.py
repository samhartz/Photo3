...
def run(species, sType, capOn, weatherOption, lightOption, rainOption, Duration, taInp, qaInp, phiInp, rInp, sInit):
    ...
    Rd0 =     {'T. aest': 4.93,    'P. menz': .32,    'O. ficu': .32, 'A. tequ': .32, 'S. bico': .32} #Standard Dark respiration at 25 C (\[Mu]mol/(m^2s))
    ...
    bp = 1.98 #Response of Vpmax to water stress, species-dependent
    dp = 2.92 #Response of Vpmax to water stress, species-dependent (MPa)
    ...
    def Vp(psi_l, ci):
        ...
        Vpmax = Vpmax0*exp(-(-psi_l/dp)**bp)
        return min((ci*Vpmax)/(ci + Kp), Vpr)
    def CbsNew(i, psi_l):
        """CO2 concentration in bundle sheath cell (ppm)"""
        return (Vp(psi_l, Cm[i]) - An(phi(tch(i)), psi_l, Tla[i], Cbs[i], i))/gbs + Cm[i]
